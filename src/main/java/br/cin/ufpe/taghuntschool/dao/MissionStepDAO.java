...
@Named
@RequestScoped
public class MissionStepDAO extends GenericDAO<MissionStep>{

	public MissionStepDAO(){
		super(MissionStep.class);
	}
	
	public MissionStep findByThingName(String thingName) {
		
		Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put("thingName", thingName);     

        List<MissionStep> missions = super.findResultsByParameter(MissionStep.FIND_BY_THING_NAME, parameters);
        if(!missions.isEmpty()){
	...
	public MissionStep findByThingKey(String thingKey) {
		
		Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put("thingKey", thingKey);     

        List<MissionStep> missions = super.findResultsByParameter(MissionStep.FIND_BY_THING_KEY, parameters);
        if(!missions.isEmpty()){
        	return missions.get(0);
        }
        return null;
