...
import br.cin.ufpe.taghuntschool.dao.StudentMissionDAO;
...
@Named
@RequestScoped
public class StudentMissionService {
	
	@Autowired
	@Inject
	StudentMissionDAO studentMissionDAO;

	public List<StudentMission> findByMission(Mission mission) {
		return studentMissionDAO.findByMission(mission);
	}
	public StudentMission findByStudentAndMission(GeneralUsr student, Mission mission) {
		return studentMissionDAO.findByStudentAndMission(student, mission);
	}

	@Transactional
    public StudentMission update(StudentMission studentMission) throws ExceptionTaghuntSchool{
		return studentMissionDAO.update(studentMission);
	}	
	@Transactional
    public void create(StudentMission studentMission) throws ExceptionTaghuntSchool{
		studentMissionDAO.update(studentMission);
	}
}
